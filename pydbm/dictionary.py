...
class Dictionary(meta.Types, meta.Group, utils.Utils):
        ...
        for dtype in np.unique(self.atoms["type"]):
            ...
            v = np.unique(
                self.atoms[inds][
                    [
                        a[0]
                        for a in self.atomGenTable[dtype].dictionaryType
                        if a[0] not in ["omega", "onset"]
                    ]
                ]
            )
            ...
            for ti, t in enumerate(v):
                ...
                if winargs:
                    BD.addBlock(
                        ...
                        **dict(
                            zip(
                                self.atomGenTable[dtype].winargs,
                                v[self.atomGenTable[dtype].winargs][ti],
                            )
                        )
                    )
                else:
                    BD.addBlock(
                        dtype, v[ti]["duration"], v[ti]["chirp"], omegas, onsets
                    )
    ...
    def writeSDIF(self, outpath):
        ...
        f.add_NVT(
            {"date": time.asctime(time.localtime()), "sample rate": self.sampleRate}
        )
        f.add_frame_type(
            self.sdifType,
            "XGAB NewXGAB, XFOF NewXFOF, XGMA NewXGMA, XDMP NewXDMP, XSGR NewXSGR",
        )
        ...
        if any(self.atoms["type"] == "soundgrain"):
            f.add_NVT(
                dict(
                    zip(
                        [str(k) for k in range(len(self.corpus.corpora))],
                        self.corpus.corpora,
                    )
                )
            )
        ...
        while n < self.num():
            ...
            for ind in np.where(self.atoms["onset"] == t)[0]:
                ...
                if N["type"] == "gabor":
                    frame.add_matrix(
                        "XGAB",
                        np.array([[N["onset"], N["duration"], N["omega"], N["chirp"]]]),
                    )
    ...
    def addRegion(
        ...
        **kwargs
        ...
        sec = (np.linspace(minsamp, maxsamp, num=sampsampling, endpoint=True)).astype(
            int
        )
        freq = np.linspace(minomega, maxomega, num=omegasampling, endpoint=True)

        d = np.zeros(
            sampsampling * omegasampling, dtype=self.atomGenTable[atype].dictionaryType
        )
        d["type"] = atype
    ...
    def addPartial(
        self, dtype, partial, scale, mcentwidth, tsampling, sampling, **kwargs
    ):

        d = np.zeros(
            np.ceil(len(partial["time"]) / float(tsampling)) * sampling,
        ...
        while ind < len(N):
            ...
            for dm in (
                np.linspace(
                    freq - (freq * 2 ** (mcentwidth / 1200.0) - freq),
                    freq * 2 ** (mcentwidth / 1200.0),
                    num=sampling,
                    endpoint=True,
                )
                / self.sampleRate
    ...
    def addTransient(
        ...
        **kwargs
        ...
        sca = (
            np.linspace(minscale, maxscale, num=scalesampling, endpoint=True)
        ).astype(int)
        freq = np.linspace(minomega, maxomega, num=omegasampling, endpoint=True)

        d = np.zeros(
            scalesampling * omegasampling, dtype=self.atomGenTable[dtype].dictionaryType
        )
        d["type"] = dtype
    ...
    def descent(self, signal, invec, vecsize, maxit, upwidth, paramsd, threshd, globd):
        ...
        k = [
            key
            for key in paramsd.keys()
            if key in [dd[0] for dd in self.atomGenTable[invec["type"]].dictionaryType]
        ]
        ...
        plist = [
            np.array(
                [
                    invec[key] - paramsd[key][0]
                    if invec[key] - paramsd[key][0] > globd[key][0]
                    else globd[key][0],
                    invec[key] + paramsd[key][1]
                    if invec[key] + paramsd[key][1] < globd[key][1]
                    else globd[key][1],
                ]
            )
            for key in k
        ...
        while (c < maxit) and (
            any(
                [abs(plist[p][0] - plist[p][1]) > threshs[p] for p in range(len(plist))]
            )
        ):
            ...
            alist = dict(
                zip(
                    ...
                    [
                        np.unique(
                            np.linspace(
                                min(p), max(p), num=vecsize, endpoint=True
                            ).astype(type(min(p)))
                        )
                        for p in plist
                    ],
            ...
            for pind, param in enumerate(cart):
                ...
                ad = self.atomGenTable[dnow["type"]].gen(
                    0.0, *[dnow[arg] for arg in self.atomGenTable[dnow["type"]].genargs]
                )
                ads = self.atomGenTable[dnow["type"]].gen(
                    np.pi / 2,
                    *[dnow[arg] for arg in self.atomGenTable[dnow["type"]].genargs]
                )
                ad /= linalg.norm(ad)
                ads /= linalg.norm(ads)
                ...
                maxc[pind] = np.sqrt(
                    np.inner(
                        ad, signal[dnow["onset"] : dnow["onset"] + dnow["duration"]]
                    )
                    ** 2
                    + np.inner(
                        ads, signal[dnow["onset"] : dnow["onset"] + dnow["duration"]]
                    )
                    ** 2
                )

            coor = np.unravel_index(
                np.argmax(np.reshape(maxc, tuple([len(p) for p in alist.values()]))),
                tuple([len(p) for p in alist.values()]),
            )

            plist = [
                np.array(
                    [
                        alist.values()[p][coor[p] - upwidth]
                        if coor[p] - upwidth >= 0
                        else alist.values()[p][0],
                        alist.values()[p][coor[p] + upwidth]
                        if coor[p] + upwidth < len(alist.values()[p])
                        else alist.values()[p][len(alist.values()[p]) - 1],
                    ]
                )
    ...
    def mp(self, signal, cmax, srr_thresh):
        ...
        while (c_cnt < cmax) and (srr <= srr_thresh):
            for cnt in up_ind:
                atom1 = self.atomGenTable[self.atoms["type"][cnt]].gen(
                    0.0,
                    *[
                        self.atoms[cnt][arg]
                        for arg in self.atomGenTable[self.atoms["type"][cnt]].genargs
                    ]
                )
                atom2 = self.atomGenTable[self.atoms["type"][cnt]].gen(
                    np.pi / 2,
                    *[
                        self.atoms[cnt][arg]
                        for arg in self.atomGenTable[self.atoms["type"][cnt]].genargs
                ...
                a1 = np.inner(
                    atom1,
                    signal[
                        self.atoms["onset"][cnt] : self.atoms["onset"][cnt]
                        + self.atoms["duration"][cnt]
                    ],
                ) / linalg.norm(atom1)
                a2 = np.inner(
                    atom2,
            ...
            atom = self.atomGenTable[self.atoms["type"][indx]].gen(
                phase,
                *[
                    self.atoms[indx][arg]
                    for arg in self.atomGenTable[self.atoms["type"][indx]].genargs
                ]
            )
            ...
            signal[
                self.atoms["onset"][indx] : self.atoms["onset"][indx]
                + self.atoms["duration"][indx]
            ] -= (atom * mag)
            out[
                self.atoms["onset"][indx] : self.atoms["onset"][indx]
                + self.atoms["duration"][indx]
            ...
            for param in [
                P[0]
                for P in self.atomGenTable[self.atoms[indx]["type"]].dictionaryType
                if P[0] not in fixedarg
            ]:
            ...
            up_ind = np.union1d(
                np.intersect1d(
                    np.where(
                        self.atoms["onset"] + self.atoms["duration"]
                        < self.atoms[indx]["onset"] + self.atoms[indx]["duration"]
                    )[0],
                    np.where(
                        self.atoms["onset"] + self.atoms["duration"]
                        >= self.atoms[indx]["onset"]
                    )[0],
                ),
                np.intersect1d(
                    np.where(
    ...
    def mp_descent(
        self, signal, cmax, srr_thresh, vecsize, maxit, upwidth, paramsd, threshd, globd
    ):
        ...
        while (c_cnt < cmax) and (srr <= srr_thresh):
            ...
            opt_param = self.descent(
                signal,
                self.atoms[indx].copy(),
                vecsize,
                maxit,
                upwidth,
                paramsd,
            ...
            g = self.atomGenTable[dnow["type"]].gen(
                0.0, *[dnow[arg] for arg in self.atomGenTable[dnow["type"]].genargs]
            )
            g /= linalg.norm(g)
            gs = self.atomGenTable[dnow["type"]].gen(
                np.pi / 2,
                *[dnow[arg] for arg in self.atomGenTable[dnow["type"]].genargs]
            )
            gs /= linalg.norm(gs)
            ...
            mag = np.sqrt(
                np.inner(g, signal[dnow["onset"] : dnow["onset"] + dnow["duration"]])
                ** 2
                + np.inner(gs, signal[dnow["onset"] : dnow["onset"] + dnow["duration"]])
                ** 2
            )
            phase = np.arctan2(
                np.inner(gs, signal[dnow["onset"] : dnow["onset"] + dnow["duration"]]),
            ...
            atom = self.atomGenTable[dnow["type"]].gen(
                phase, *[dnow[arg] for arg in self.atomGenTable[dnow["type"]].genargs]
            )
            ...
            for param in [
                P[0]
                for P in self.atomGenTable[dnow["type"]].dictionaryType
                if P[0] not in fixedarg
            ]:
            ...
            up_ind = np.union1d(
                np.intersect1d(
                    np.where(
                        self.atoms["onset"] + self.atoms["duration"]
                        < dnow["onset"] + dnow["duration"]
                    )[0],
                    np.where(
                        self.atoms["onset"] + self.atoms["duration"] >= dnow["onset"]
                    )[0],
                ),
                np.intersect1d(
                    np.where(self.atoms["onset"] < dnow["onset"] + dnow["duration"])[0],
...
class SpecDictionary(Dictionary, meta.Spectral, utils.Utils):
    ...
    def addNote(self, dtype, midicents, scales, onsets, **kwargs):
        ...
        d = np.zeros(
            sum([len(o) for o in onsets]), dtype=self.atomGenTable[dtype].dictionaryType
        )
        d["type"] = dtype
    ...
    def mp(self, signal, cmax, srr_thresh, tolmidicents, maxPeaks, dBthresh, overspec):
        ...
        while (c_cnt < cmax) and (srr <= srr_thresh) and (abs(last_srr - srr) > 0):
            for cnt in up_ind:
                ...
                win = self.atomGenTable[self.atoms["type"][cnt]].window(
                    self.atoms["duration"][cnt], **winargs
                )
                X = fftpack.fft(
                    signal[
                        self.atoms["onset"][cnt] : self.atoms["onset"][cnt]
                ...
                L, V = self.pickPeaks(
                    ...
                    maxPeaks,
                    dBthresh,
                    self.atomGenTable[self.atoms["type"][cnt]].enbw(
                        self.atoms["duration"][cnt], **winargs
                    ),
                    self.atoms["duration"][cnt] * overspec,
                    self.atoms["type"][cnt],
                    **winargs
                )
                ...
                L = L[
                    (L != 0.0)
                    & (
                        self.hz2midi(L / self.atoms["duration"][cnt] * self.sampleRate)
                        > self.hz2midi(self.atoms["omega"][cnt] * self.sampleRate) - tol
                    )
                ]
                ...
                f0 = (
                    L[
                        np.argmin(
                            abs(
                                L / self.atoms["duration"][cnt]
                                - self.atoms["omega"][cnt]
                            )
                        )
                    ]
                    / self.atoms["duration"][cnt]
                ...
                dist = abs(
                    self.hz2midi(f0 * self.sampleRate)
                    - self.hz2midi(self.atoms["omega"][cnt] * self.sampleRate)
                )
                ...
                inds = [
                    i
                    for (i, val) in enumerate(fnote)
                    if np.min(abs(tnote - val)) <= tol
                ]
                ...
                if len(inds) > 0:
                    ...
                    for ii, i in enumerate(inds):
                        ...
                        a1 = self.atomGenTable[dnow["type"]].gen(
                            0.0,
                            *[
                                dnow[arg]
                                for arg in self.atomGenTable[dnow["type"]].genargs
                            ]
                        )
                        a1 /= linalg.norm(a1)
                        a2 = self.atomGenTable[dnow["type"]].gen(
                            np.pi / 2,
                            *[
                                dnow[arg]
                                for arg in self.atomGenTable[dnow["type"]].genargs
                        ...
                        c1 = np.inner(
                            a1, signal[dnow["onset"] : dnow["onset"] + dnow["duration"]]
                        )
                        c2 = np.inner(
                            a2, signal[dnow["onset"] : dnow["onset"] + dnow["duration"]]
                        )
            ...
            X = fftpack.fft(
                signal[dnow["onset"] : dnow["onset"] + dnow["duration"]] * win
            )
            ...
            L, V = self.pickPeaks(
                ...
                maxPeaks,
                dBthresh,
                self.atomGenTable[dnow["type"]].enbw(dnow["duration"], **winargs),
                dnow["duration"] * overspec,
                dnow["type"],
                **winargs
            )
            L, V, P = self.interpolateValues(
            ...
            L = L[
                (L != 0.0)
                & (
                    self.hz2midi(L / dnow["duration"] * self.sampleRate)
                    > self.hz2midi(dnow["omega"] * self.sampleRate) - tol
                )
            ]
            ...
            # fundamental
            f0 = (
                L[np.argmin(abs(L / dnow["duration"] - dnow["omega"]))]
                / dnow["duration"]
            )
            dist = abs(
                self.hz2midi(f0 * self.sampleRate)
                - self.hz2midi(dnow["omega"] * self.sampleRate)
            )
            ...
            tnote = self.hz2midi(f0 * np.arange(1, maxPeaks) * self.sampleRate)
            inds = [
                i for (i, val) in enumerate(fnote) if np.min(abs(tnote - val)) <= tol
            ]
            ...
            for ii, i in enumerate(inds):
                ...
                a1 = self.atomGenTable[dnow["type"]].gen(
                    phi, *[dnow[arg] for arg in self.atomGenTable[dnow["type"]].genargs]
                )
                a1 /= linalg.norm(a1)
    ...
    def mp2(self, signal, cmax, srr_thresh, tolmidicents, maxPeaks, dBthresh, overspec):
        ...
        while (c_cnt < cmax) and (srr <= srr_thresh) and (abs(last_srr - srr) > 0):
            ...
            atom = np.hstack(
                np.array((np.matrix(pmag) * np.vstack(lsq_m)))
            ) + np.hstack(np.array((np.matrix(pphase) * np.vstack(lsq_p))))
...
class InstrumentDictionary(data.InstrumentSubspace, Dictionary):
    ...
    def addPitch(self, pitchcents, dtype, scale, timearray, **kwargs):
        ...
        M = np.array(
            [
                self.midi2hz(k / 100.0) / float(self.sampleRate)
                for k in range(
                    pitchcents - tolcents, pitchcents + tolcents, centssampling
                )
            ]
        )
        A = np.zeros(
            ...
            * sum(
                [
                    len(p)
                    for p in self.partialAmplitudes[pitchcents]
                    if len(p) >= mincomponents and len(p) <= maxcomponents
                ]
            ),
            dtype=self.atomGenTable[dtype].dictionaryType,
        )
                ...
                for array_i, array in enumerate(self.partialAmplitudes[pitchcents]):
                    if len(array) >= mincomponents and len(array) <= maxcomponents:
                        ...
                        for a_i, a in enumerate(array):
                            ...
                            A["k_mag"][c] = a
                            A["k_phase"][c] = self.partialPhases[pitchcents][array_i][
                                a_i
                            ]  # this just sets a value, the phase will have to be determined in the pursuit
                            A["omega"][c] = m * (
                                self.partialFrequencies[pitchcents][array_i][a_i] / f0
                            )
                            A["index"][c][1] = cc
                            A["index"][c][0] = a_i
                            A["onset"][c] = t
    ...
    def normalizeStructures(self):
        ...
        for i in np.unique(self.atoms["index"][1]):
            ...
            self.atoms[inds]["k_mag"] *= 1.0 / np.sqrt(
                np.sum(self.atoms[inds]["k_mag"] ** 2)
            )
    ...
    def mp(self, signal, cmax, srr_thresh):
        ...
        # place to hold analysis values
        f0i = np.where(self.atoms["index"][0] == 0)[
            0
        ]  # we only really need to search the f0s, since everything is built from there
        ...
        while (c_cnt < cmax) and (srr <= srr_thresh):
            for cnt in up_ind:
                ...
                compi = np.intersect1d(
                    np.where(self.atoms["source"] == self.atoms[di]["source"])[0],
                    np.where(self.atoms["index"][1] == self.atoms[di]["index"][1])[0],
                )
                ...
                for cind in compi:
                    ...
                    a__1 = self.atomGenTable[self.atoms[cind]["type"]].gen(
                        0.0,
                        *[
                            self.atoms[cind][arg]
                            for arg in self.atomGenTable[
                                self.atoms[cind]["type"]
                            ].genargs
                        ]
                    )
                    ...
                    c__1 = np.inner(
                        a__1,
                        signal[
                            self.atoms[cind]["onset"] : self.atoms[cind]["onset"]
                            + self.atoms[cind]["duration"]
                        ],
                    )

                    a__2 = self.atomGenTable[self.atoms[cind]["type"]].gen(
                        np.pi / 2,
                        *[
                            self.atoms[cind][arg]
                            for arg in self.atomGenTable[
                    ...
                    self.atoms[cind]["k_phase"] = np.arctan2(
                        c__2, c__1
                    )  # overwrite the phase value to get it back later

                    a__1 = self.atomGenTable[self.atoms[cind]["type"]].gen(
                        self.atoms[cind]["k_phase"],
                        *[
                            self.atoms[cind][arg]
                            for arg in self.atomGenTable[
                                self.atoms[cind]["type"]
                    ...
                    a__2 = self.atomGenTable[self.atoms[cind]["type"]].gen(
                        self.atoms[cind]["k_phase"] + np.pi / 2,
                        *[
                            self.atoms[cind][arg]
                            for arg in self.atomGenTable[
                                self.atoms[cind]["type"]
                ...
                c1 = np.inner(
                    a_1,
                    signal[
                        self.atoms[di]["onset"] : self.atoms[di]["onset"]
                        + self.atoms[di]["duration"]
                    ],
                )
                c2 = np.inner(
                    a_2,
            ...
            for cindi, cind in enumerate(compi):
                a = self.atomGenTable[self.atoms[cind]["type"]].gen(
                    self.atoms[cind]["k_phase"] + phase,
                    *[
                        self.atoms[cind][arg]
                        for arg in self.atomGenTable[self.atoms[cind]["type"]].genargs
                    ]
                )
            ...
            up_ind = np.union1d(
                np.intersect1d(
                    np.where(
                        self.atoms["onset"][f0i] + self.atoms["duration"][f0i]
                        < self.atoms[di]["onset"] + self.atoms[di]["duration"]
                    )[0],
                    np.where(
                        self.atoms["onset"][f0i] + self.atoms["duration"][f0i]
                        >= self.atoms[di]["onset"]
...
class Block(meta.Types):
    ...
    def __init__(self, dtype, scale, chirp, omegas, onsets, **winargs):
        ...
        self.genargs = [
            ...
            for key in [
                k
                for k in self.AtomGen.genargs
                if k not in ["duration", "omega", "chirp"]
            ]
        ]
        self.gen = lambda phi, omega: self.AtomGen.gen(
            phi, self.scale, omega, chirp, *self.genargs
        )
...
class BlockDictionary(meta.Types, utils.Utils):
    ...
    def addPartialBlock(self, Partial, foversampfact, dtype, scale, chirp, **winargs):
        ...
        t = interpolate.interp1d(
            x, np.floor(Partial.array["time"] * self.sampleRate).astype(np.int)
        )
        f = interpolate.interp1d(x, Partial.array["frequency"] / self.sampleRate)
        newx = np.linspace(
            0.0,
            len(Partial.array["time"]) - 1,
            len(Partial.array["time"]) * foversampfact,
            endpoint=False,
        )
    ...
    def mp(self, signal, cmax, srr_thresh):
        ...
        while (c_cnt < cmax) and (srr <= srr_thresh):
            ...
            for ib, block in enumerate(self.blocks):
                ...
                Nfft = 2 ** self.nextpow2(
                    block.scale
                    + (block.onsets[up_inds[ib][-1]] - block.onsets[up_inds[ib][0]])
                )  # a large fft
                S = fftpack.fft(
                    signal[
                        block.onsets[up_inds[ib][0]] : block.onsets[up_inds[ib][-1]]
                    ],
                    Nfft,
                )
                ...
                for iom, omega in enumerate(block.omegas):
                    ...
                    cc = np.real(
                        fftpack.ifft(np.conj(fftpack.fft(a_c, Nfft)) * S)[
                            block.onsets[up_inds[ib]] - min(block.onsets[up_inds[ib]])
                        ]
                    )
                    cs = np.real(
                        fftpack.ifft(np.conj(fftpack.fft(a_s, Nfft)) * S)[
                            block.onsets[up_inds[ib]] - min(block.onsets[up_inds[ib]])
                        ]
                    )
            ...
            signal[
                self.blocks[nb].onsets[nind[1]] : self.blocks[nb].onsets[nind[1]]
                + self.blocks[nb].scale
            ] -= (atom * nmag)
            out[
                self.blocks[nb].onsets[nind[1]] : self.blocks[nb].onsets[nind[1]]
                + self.blocks[nb].scale
            ...
            for ui, u in enumerate(up_inds):
                up_inds[ui] = np.union1d(
                    np.intersect1d(
                        np.where(
                            self.blocks[ui].onsets + self.blocks[ui].scale
                            < self.blocks[nb].onsets[nind[1]] + self.blocks[nb].scale
                        )[0],
                        np.where(
                            self.blocks[ui].onsets + self.blocks[ui].scale
                            >= self.blocks[nb].onsets[nind[1]]
                        )[0],
                    ),
                    np.intersect1d(
                        np.where(
                            self.blocks[ui].onsets
                            < self.blocks[nb].onsets[nind[1]] + self.blocks[nb].scale
                        )[0],
                        np.where(
                            self.blocks[ui].onsets >= self.blocks[nb].onsets[nind[1]]
                        )[0],
                    ),
                )
                mags[ui][0:, up_inds[ui]] = 0.0
            c_cnt += 1
...
class SoundgrainDictionary(meta.Group, meta.IO, utils.Utils):
    ...
    def addPolygon(self, signal, win, Poly, nbins, hop, nbest):
        ...
        X_rms = np.sqrt(np.sum((2.0 * abs(X[0 : nbins / 2, 0:]) / win / cg) ** 2))
        Xsub_rms = np.sqrt(
            np.sum(
                (
                    2.0
                    * abs(X[0 : nbins / 2, np.unique(Poly.polyHull["hop"])])
                    / win
                    / cg
                )
                ** 2
        ...
        for cind, C in enumerate(self.SoundDatabase.corpora):
            ...
            for ind, name in enumerate(C.soundfiles):
                ...
                hull = Poly.polyHull[
                    np.where(
                        Poly.polyHull["hop"] - min(Poly.polyHull["hop"])
                        < np.shape(Y)[1]
                    )[0]
                ]
                    ...
                    np.sum(
                        (
                            ...
                            * abs(
                                Y[
                                    0 : nbins / 2,
                                    np.unique(hull["hop"]) - min(hull["hop"]),
                                ]
                            )
                            / win
                            / cg
    ...
    def addCorpus(self, onsets, corpus_index):
        ...
        for n in N:
            ...
            for i in range(self.SoundDatabase.corpora[corpus_index].num()):
                ...
                da["file_index"][p] = self.SoundDatabase.corpora[
                    corpus_index
                ].fileDescriptors["file_index"][i]
                da["onset"][p] = n
                da["duration"][p] = self.SoundDatabase.corpora[
                    corpus_index
                ].fileDescriptors["length"][i]
                da["norm"][p] = self.SoundDatabase.corpora[
                    corpus_index
                ].fileDescriptors["norm"][i]
    ...
    def mp(self, signal, cmax, srr_thresh):
        ...
        while (c_cnt < cmax) and (srr <= srr_thresh):
            for cnt in up_ind:
                ...
                atom = self.readAudio(
                    self.SoundDatabase.corpora[
                        self.atoms["corpus_index"][cnt]
                    ].directory
                    + "/"
                    + self.SoundDatabase.corpora[
                        self.atoms["corpus_index"][cnt]
                    ].soundfiles[self.atoms["file_index"][cnt]]
                )[0]
            ...
            atom = (
                self.readAudio(
                    self.SoundDatabase.corpora[
                        self.atoms["corpus_index"][indx]
                    ].directory
                    + "/"
                    + self.SoundDatabase.corpora[
                        self.atoms["corpus_index"][indx]
                    ].soundfiles[self.atoms["file_index"][indx]]
                )[0]
                / self.atoms["norm"][indx]
            )
            ...
            # Measure the change
            srr = 10 * np.log10(linalg.norm(out) ** 2 / linalg.norm(signal) ** 2)
            up_ind = np.intersect1d(
                np.where(self.atoms["onset"] >= self.atoms["onset"][indx] - max_scale)[
                    0
                ],
                np.where(self.atoms["onset"] < self.atoms["onset"][indx] + max_scale)[
                    0
    ...
    def tvmp(self, signal, cmax, srr_thresh, globscalar):
        ...
        while (c_cnt < cmax) and (srr <= srr_thresh):
            ...
            a = max_mag[indx]
            atom = self.readAudio(
                self.SoundDatabase.corpora[self.atoms["corpus_index"][indx]].directory
                + "/"
                + self.SoundDatabase.corpora[
                    self.atoms["corpus_index"][indx]
                ].soundfiles[self.atoms["file_index"][indx]]
    ...
    def mpc(self, signal, cmax, srr_thresh, maxsimul, mindistance):
        ...
        while (c_cnt < cmax) and (srr <= srr_thresh):
            ...
            if c_cnt == 0:
                ...
                up_ind = np.intersect1d(
                    np.where(
                        self.atoms["onset"] >= self.atoms["onset"][indx] - max_scale
                    )[0],
                    np.where(
                        self.atoms["onset"] < self.atoms["onset"][indx] + max_scale
                    )[0],
                ...
                for magi in mag_sort:
                    constraint1 = (
                        sum(
                            np.where(
                                M.atoms["onset"][0:c_cnt] == self.atoms["onset"][magi]
                            )[0]
                        )
                        <= maxsimul
                    )
                    constraint2 = (
                        all(
                            abs(M.atoms["onset"][0:c_cnt] - self.atoms["onset"][magi])
                            >= mindistance
                        )
                        or self.atoms["onset"][magi] in M.atoms["onset"][0:c_cnt]
                    )
    ...
    def addCorpus(self, onsets, corpus_index):
        ...
        for n in N:
            ...
            for i in range(self.SoundDatabase.corpora[corpus_index].num()):
                ...
                da["midicent"][p] = self.SoundDatabase.corpora[
                    corpus_index
                ].fileDescriptors["midicent"][i]
                da["velocity"][p] = self.SoundDatabase.corpora[
                    corpus_index
                ].fileDescriptors["velocity"][i]
                ...
                for magi in mag_sort:
                    ...
                    if tv:
                        if indx in last_indxs:
                            print(
                                "This atom has already been used... continuing search..."
                            )
                            continue
                    ...
                    c1 = (
                        numberAtomsNow
                        <= constraints["mxsa"]["max_simultaneous_atoms"][cind]
                    )
                    ...
                    if c1:
                        ...
                        c2 = (
                            all(
                                abs(
                                    M.atoms["onset"][0:c_cnt]
                                    - self.atoms["onset"][magi]
                                )
                                >= constraints["mntd"]["min_time_distance"][cind]
                            )
                            or self.atoms["onset"][magi] in M.atoms["onset"][0:c_cnt]
                        )
                        ...
                        if c2:
                            ...
                            if prev == None:
                                ...
                                if len(mxprev_o_i) > 1:
                                    prev_atom = M.atoms[mxprev_o_i][
                                        np.argmin(
                                            abs(
                                                M.atoms[mxprev_o_i]["midicent"]
                                                - self.atoms[magi]["midicent"]
                                            )
                                        )
                                    ]
                                    ...
                                    mxprev_o = prev_o[1]
                                    mxprev_o_i = np.where(M.atoms["onset"] == mxprev_o)[
                                        0
                                    ]
                                    if len(mxprev_o_i) > 1:
                                        prev_atom = M.atoms[mxprev_o_i][
                                ...
                                den = (
                                    float(
                                        self.atoms[magi]["onset"] - prev_atom["onset"]
                                    )
                                    / self.sampleRate
                                    * 1000
                                )
                                ...
                                c3_3v = (
                                    abs(
                                        self.atoms[magi]["velocity"]
                                        - prev_atom["velocity"]
                                    )
                                    / den
                                )
                                print(
                                    'The current "backward" midicent slope is %f'
                                    % c3_1v
                                )
                                print(
                                    'The current "backward" velocity slope is %f'
                                    % c3_3v
                                )

                                c3_1 = (
                                    c3_1v
                                    <= constraints["mxps"]["max_pitch_slope"][cind]
                                )
                                c3_3 = (
                                    c3_3v
                                    <= constraints["mxvs"]["max_velocity_slope"][cindv]
                                )
                            ...
                            if following == None:
                                ...
                                mxfollowing_o_i = np.where(
                                    M.atoms["onset"] == mxfollowing_o
                                )[0]
                                ...
                                if len(mxfollowing_o_i) > 1:
                                    following_atom = M.atoms[mxfollowing_o_i][
                                        np.argmin(
                                            abs(
                                                M.atoms[mxfollowing_o_i]["midicent"]
                                                - self.atoms[magi]["midicent"]
                                            )
                                        )
                                    ]
                                ...
                                den = (
                                    float(
                                        following_atom["onset"]
                                        - self.atoms[magi]["onset"]
                                    )
                                    / self.sampleRate
                                    * 1000
                                )
                                ...
                                c3_4v = (
                                    abs(
                                        following_atom["velocity"]
                                        - self.atoms[magi]["velocity"]
                                    )
                                    / den
                                )
                                print(
                                    'The current "forward" midicent slope is %f' % c3_2v
                                )
                                print(
                                    'The current "forward" velocity slope is %f' % c3_4v
                                )
                                c3_2 = (
                                    c3_2v
                                    <= constraints["mxps"]["max_pitch_slope"][cind]

...
class Book(meta.Types, meta.Group, utils.Utils):
    ...
    def writeSDIF(self, outpath):
        ...
        f.add_matrix_type(
            "XFOM", "index, onset, scale, omega, chirp, rise, decay, mag, phase, norm"
        )
        f.add_matrix_type(
            "XGMM",
        ...
        f.add_matrix_type(
            "XDMM", "index, onset, scale, omega, chirp, damp, mag, phase, norm"
        )
    ...
    def synthesize(self, synthtype="default", kwargs={}):
        ...
        if synthtype == "default":
            for a in self.atoms:
                a_ = self.atomGenTable[a["type"]].gen(
                    a["phase"],
                    *[a[arg] for arg in self.atomGenTable[a["type"]].genargs]
                )
                a_ /= linalg.norm(a_)
            ...
            for a in self.atoms:
                a_ = self.atomGenTable[a["type"]].genFM(
                    a["phase"],
                    *[a[arg] for arg in self.atomGenTable[a["type"]].genargs],
                    **kwargs
                )
            ...
            for a in self.atoms:
                ...
                a_ /= linalg.norm(a_)
                out[a["onset"] : a["onset"] + a["duration"]] += (
                    a_ * a["k_mag"] * a["mag"]
                )
    ...
    def wivigram(self, hop, fftsize, plot=False):
        ...
        W = np.zeros(
            (
                fftsize,
                np.ceil(max(self.atoms["onset"] + self.atoms["duration"]) / float(hop)),
            )
        )

        for a in self.atoms:
            a_ = self.atomGenTable[a["type"]].gen(
                a["phase"], *[a[arg] for arg in self.atomGenTable[a["type"]].genargs]
            )
            a_ /= linalg.norm(a_)
            a_ *= a["mag"]
            ...
            W[
                0:fftsize,
                np.floor(a["onset"] / float(hop)) : np.floor(a["onset"] / float(hop))
                + np.shape(w)[1],
            ] += np.abs(w)
    ...
    def crosscorrelate(
        self, num, compfunc=lambda x, y: np.abs(np.inner(np.conj(x), y))
    ):
        ...
        for i, a in enumerate(S):
            aa = (
                a["mag"]
                * np.exp(2 * np.pi * 1j * a["omega"] * np.arange(a["duration"]))
                * np.exp(1j * a["phase"])
                * self.atomGenTable[a["type"]].window(a["duration"])
            )
            aa /= linalg.norm(aa)
            ...
            for q, b in enumerate(S):
                ab = (
                    b["mag"]
                    * np.exp(2 * np.pi * 1j * b["omega"] * np.arange(b["duration"]))
                    * np.exp(1j * b["phase"])
                    * self.atomGenTable[b["type"]].window(b["duration"])
                )
                ab /= linalg.norm(ab)
    ...
    def agglomerate(self, M, thresh):
        ...
        a = [
            np.array([k for k in range(np.shape(M_)[1]) if M_[j][k] == 1])
            for j in range(np.shape(M_)[0])
        ]
        La = np.array([])
        Na = []
    ...
    def writeMolecules(self, N, outdir):
        ...
        for i, mol in enumerate(Na):
            ...
            for ind in mol:
                a_ = self.atomGenTable[self.atoms[ind]["type"]].gen(
                    self.atoms[ind]["phase"],
                    *[
                        self.atoms[ind][arg]
                        for arg in self.atomGenTable[self.atoms[ind]["type"]].genargs
                    ]
                )
                a_ /= linalg.norm(a_)
                out[
                    self.atoms["onset"][ind] : self.atoms["onset"][ind]
                    + self.atoms["duration"][ind]
                ] += (a_ * self.atoms[ind]["mag"])
...
class SpectralBook(Book):
    ...
    def selfSimilarity(self):
        ...
        for i in K:
            ...
            for j in K:
                ...
                m = max([len(a), len(b)])
                I[i, j] = self.itakuraSaito(
                    self.powerSpectrum(a, m), self.powerSpectrum(b, m)
                )
    ...
    def writeMolecules(self, N, outdir):
        ...
        for i, mol in enumerate(N):
            ...
            for ind in mol:
                ...
                out[
                    self.atoms["onset"][q[0]] : self.atoms["onset"][q[0]]
                    + self.atoms["duration"][q[0]]
                ] += self.structSynth(ind)
    ...
    def pianoroll(self, dsfactor):
        ...
        out = np.zeros(
            (
                len(self.model) / dsfactor,
                max(self.hz2midi(self.atoms["omega"] * self.sampleRate)),
            )
        )
        s = np.unique(self.atoms["index"][1])
        ...
        for k in s:
            ...
            out[
                np.floor((self.atoms["onset"][inds[0]]) / dsfactor) : np.floor(
                    (self.atoms["onset"][inds[0]]) / dsfactor
                )
                + np.floor(self.atoms["duration"][inds[0]] / dsfactor),
                f0,
            ] += np.ones(np.floor(self.atoms["duration"][inds[0]] / dsfactor)) * sum(
                self.atoms["mag"][inds]
            )
...
class SoundgrainBook(meta.Group, meta.IO):
    ...
    def synthesize(self):
        ...
        for i in range(len(self.atoms)):
            atom = (
                self.readAudio(
                    self.SoundDatabase.corpora[self.atoms["corpus_index"][i]].directory
                    + "/"
                    + self.SoundDatabase.corpora[
                        self.atoms["corpus_index"][i]
                    ].soundfiles[self.atoms["file_index"][i]]
                )[0]
                / self.atoms["norm"][i]
            )
            out[
                self.atoms["onset"][i] : self.atoms["onset"][i]
                + self.atoms["duration"][i]
            ] += (atom * self.atoms["mag"][i])
    ...
    def writeSDIF(self, outpath, labeled=False):
        ...
        f.add_NVT(
            dict(
                [("TableName", "CorpusDirectories")]
                + zip(
                    [str(k) for k in uc],
                    [C.directory for C in [self.SoundDatabase.corpora[c] for c in uc]],
                )
            )
        )
        ...
        for c in uc:
            ...
            f.add_NVT(
                dict(
                    [("TableName", "Corpus-%i" % c)]
                    + zip(
                        [str(k) for k in finds],
                        [self.SoundDatabase.corpora[c].soundfiles[k] for k in finds],
                    )
                )
            )
        ...
        self.atoms.sort(order="onset")
        f.add_matrix_type(
            "XSGM", "onset, duration, corpus_index, file_index, norm, mag"
        )
        f.add_matrix_type(
            "XSLM",
            "onset, duration, corpus_index, file_index, norm, midicents, velocity, mag",
        )
        n = 0
...
class InstrumentSoundgrainBook(SoundgrainBook):
    ...
    def crosscorrelate(
        self,
        num,
        compfunc=lambda x, y: np.sqrt(
            (x["midicent"] - y["midicent"]) ** 2 + (x["onset"] - y["onset"]) ** 2
        ),
    ):
        """Agglomerate (a subset of) the synthesis book into molecules, where
        num := number of atoms to consider
    ...
    def writeSDIF(self, outpath, labeled=True):
        ...
        self.atoms.sort(order="onset")
        f.add_matrix_type(
            "XSGM", "onset, duration, corpus_index, file_index, norm, mag, molecule"
        )
        f.add_matrix_type(
            "XSLM",

...
class Utils(object):
    ...
    def ed(self, x):
        ...
        psi = sig.hilbert(
            x
        )  # alternatively, inverse transform of X(f) with negative frequencies set to zero
    ...
    def renyiEntropy(self, alpha, beta):
        ...
        return 1.0 / (beta - 1) * np.log2(np.sum(np.abs(alpha) ** (2 * beta)))
    ...
    def hz2midi(self, cps):
        ...
        m = 69 + 12 * np.log2(cps / 440.0 + 0.0000000001)
    ...
    def midi2hz(self, midi_num, f_ref=440.0, midi_ref=69):
        ...
        return f_ref * 2 ** ((midi_num - midi_ref) / 12.0)
    ...
    def itakuraSaito(self, X, Y):
        ...
        return (1.0 / len(X)) * sum((X / Y) - np.log(X / Y) - 1)
    ...
    def lpc(self, x, order):
        ...
        S = fftpack.fft(x, (2 ** self.nextpow2(2.0 * len(x))))
    ...
    def mfcc(self, x, Fs, win, hop, nfft, CC):
        ...
        preEmp = sig.lfilter(np.array([1, -0.97]), 1, seg)
        fbank = self.trfbank(
            Fs,
            nfft,
            lowestFrequency,
        ...
        ceps = fftpack.realtransforms.dct(earMag, type=2, norm="ortho", axis=-1)[
            :, 0:CC
        ]
    ...
    def dftMatrix(self, N):
        ...
        W = np.asmatrix(1.0 / np.sqrt(N) * np.power(omega, F))
    ...
    def idct(self, X):
        ...
        N = len(X)
        w = np.sqrt(2 * N) * np.exp(1j * np.arange(N) * np.pi / (2.0 * N))

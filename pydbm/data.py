...
class InstrumentSubspace(meta.IO, meta.Spectral, utils.Utils):
    ...
    def getSpectralPeaks(
        ...
        **winargs
        ...
        self.envelopes = (
            {}
        )  # a midicents (pitch class) to list of envelope structured arrays look up table
        ...
        for corpus in trainingCorpora:
            ...
            for fn in os.listdir(corpus):
                ...
                if (fn[0] == ".") or (
                    os.path.splitext(fn)[1] not in self.informats.keys()
                ):
                    continue
                ...
                freq = self.midi2hz(midi / 100.0) / float(fs)
                minSpace = int(np.round((freq / 2.0) / (1.0 / Nfft)))
                tol = 2 * (1.0 / Nfft)
                ...
                while pin < pend:
                    ...
                    X = fftpack.fft(
                        xp[pin : pin + W]
                        * self.atomGenTable[window].window(W, **winargs),
                        Nfft,
                    )
                    R = abs(X)
                    P = np.angle(X)
                    ...
                    L, V = self.pickPeaks(
                        R, maxPeaks, dBthresh, minSpace, W, window, **winargs
                    )

                    if interp:
                        L, V, P = self.interpolateValues(
                            R, P, Nfft, L, V, W, window, **winargs
                        )
                    ...
                    a = (
                        ...
                        * V[ind]
                        / W
                        * (1.0 / self.atomGenTable[window].coherentGain(W, **winargs))
                    )  # linear amplitude values from magnitude
    ...
    def round(self, dec=3):
        ...
        for k in self.envelopes.keys():
            ...
            for omi, om in enumerate(u):
                ...
                env["amplitude"][omi] = np.mean(
                    reduce(
                        np.union1d,
                        [
                            self.envelopes[k][i]["amplitude"][b[i]]
                            for i in range(len(b))
                        ],
                    )
                )
                env["phase"][omi] = np.mean(
                    reduce(
                        np.union1d,
                        [self.envelopes[k][i]["phase"][b[i]] for i in range(len(b))],
                    )
                )
                env["omega"][omi] = om
    ...
    def normalize(self):
        ...
        for key in self.envelopes.keys():
            ...
            for a in self.envelopes[key]:
                ...
                a["amplitude"] *= 1.0 / np.sqrt(np.sum(a["amplitude"] ** 2))
...
class Corpus(meta.IO):
    ...
    def getLabelDescriptors(self):
        ...
        self.labelDescriptors = {
            "midicents": np.zeros(sum(self.num()), dtype=int),
            "velocity": np.zeros(sum(self.num()), dtype=int),
        }
...
class PartialModel(meta.IO):
    ...
    def getPartials(self):
        self.partials = [
            Partial(i, self.partialModel[np.where(self.partialModel["index"] == i)[0]])
            for i in np.unique(self.partialModel["index"])
        ]
    ...
    def partialSort(self, order="frequency"):
        ...
        inds = {
            "amplitude": np.argsort(
                [sum(abs(p.array["amplitude"])) for p in self.partials]
            )[::-1],
            "frequency": np.argsort(
                [np.mean(p.array["frequency"]) for p in self.partials]
            ),
            "duration": np.argsort(
                [
                    np.max(p.array["time"]) - np.min(p.array["time"])
        ...
        for k in range(len(self.partials)):
            ...
            for i in range(len(self.partials)):
                ...
                for n in range(min([len(a.array["time"]), len(b.array["time"])])):
                    s += abs(
                        alpha
                        * (a.array["time"][n] / max_t - b.array["time"][n] / max_t)
                    ) + abs(
                        beta
                        * (
                            a.array["frequency"][n] / max_f
                            - b.array["frequency"][n] / max_f
                        )
                    )
...
class Polygon(object):
    ...
    def readFrame(self, sdif_frame):
        ...
        if "pnts" in self.params.keys():
            self.points = np.zeros(
                len(self.params["pnts"]), dtype=[("time", float), ("frequency", float)]
            )
    ...
    def getPolyHull(self, fs, hop, fftsize):
        ...
        self.polyHull = np.zeros(
            (max_t + 1 - min_t) * (max_f + 1 - min_f),
            dtype=[("hop", int), ("bin", int)],
        )
            ...
            for m_ind, measure in enumerate(part.findall("measure")):
                ...
                for n_ind, note in enumerate(measure.findall("note")):
                    ...
                    if note.find("pitch"):
                        ...
                        name = (
                            note.find("pitch").find("step").text
                            + note.find("pitch").find("octave").text
                        )
                        self.parts[p_ind].measures[m_ind].notes[n_ind].name = name
                        self.parts[p_ind].measures[m_ind].notes[n_ind].duration = eval(
                            note.find("duration").text
                        )
                        self.parts[p_ind].measures[m_ind].notes[n_ind].type = note.find(
                            "type"
                        ).text
                        ...
                        self.parts[p_ind].measures[m_ind].notes[n_ind].midicents = (
                            self.midiLookup[name] + a
                        ) * 100

...
class Types(object):
    ...
    def __init__(self):
        ...
        self.sdifTypes = {'1TRC' : [('time', float), ('index', int), ('frequency', float), ('amplitude', float), ('phase', float)],
                             ...
                             'XSGM' : [('time', float), ('index', int), ('onset', int), ('duration', int), ('corpus_index', int), ('file_index', int), ('norm', float), ('mag', float)]}
...
class Spectral(Types):
    ...
    def spectralShapeStatistics(self, x, Nfft):
        ...
        mu = lambda fftdata, order: np.sum(np.abs(fftpack.fftfreq(Nfft))**order * np.abs(fftdata)) / np.sum(np.abs(fftdata))
        F = fftpack.fft(x * sig.hanning(len(x)), Nfft)
        
        centroid = mu(F, 1)
        spread = np.sqrt(mu(F, 2) - mu(F, 1)**2)
        skewness = (2*mu(F,1)**3 - 3*mu(F,1)*mu(F,2) + mu(F,3)) / spread**3
        kurtosis = (-3*mu(F,1)**4 + 6*mu(F,1)*mu(F,2) - 4*mu(F,1)*mu(F,3) + mu(F,4)) / spread**4 - 3
        
        return centroid, spread, skewness, kurtosis

    def pickPeaks(self, magSpectrum, nPeaks, dBthresh, minSpace, winSize, windowName, **winargs):
        ...
        wcoef = 1./self.atomGenTable[windowName].coherentGain(winSize, **winargs)
        ...
        for k in xrange(nPeaks):
            ...
            #dB
            if 20*np.log10( 2 * (ivalPP[point]+eps) / float(winSize) * wcoef) < dBthresh:
                break
    ...
    def interpolateValues(self, magnitude, phase, N, cur_loc, cur_val, W, windowName, **winargs):
        ...
        zp = N/W
        wcoef = 1./self.atomGenTable[windowName].coherentGain(W, **winargs)

        #parabolic interpolation of bin values
        leftval = magnitude[((cur_loc-1) * (cur_loc-1 > -1) + (cur_loc-1 <= -1 *1)).astype(np.int16)] #tricky indexing hack, avoids edges
        rightval = magnitude[((cur_loc+1) * (cur_loc+1 < N/2) + (cur_loc+1 >= N/2 * N/2 )).astype(np.int16)]
